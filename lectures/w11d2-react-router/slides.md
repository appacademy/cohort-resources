...
Note:
- <Link>generates an anchor tag in our HTML to a specified path

- Unlike anchor tags, which reloads the whole page, Links only reload the portion(s) of the UI that match(s) the browser's location path.

- The `to=` prop specifies the route you want to navigate the user to. This should be an existing route that you've defined and have specified a component prop. So when you link to it, the corresponding component is what renders.

- You can style this tag like any other react component with a className or id.
