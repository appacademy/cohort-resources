...
-- How many types of possessions have a total cost over $2000?
  -- type AS expensive_possession_types
SELECT
  COUNT(*) AS num_types
FROM (
      SELECT
...
SELECT
  sf_instructors.name, COUNT(*) AS num_possessions
FROM
  sf_instructors
JOIN
...
SELECT
  sf_instructors.name, ny_instructors.name
FROM
  friendships
JOIN
  sf_instructors ON friendships.sf_id = sf_instructors.id
JOIN
...
ON pod_leaders.id = instructors.pod_leader_id;