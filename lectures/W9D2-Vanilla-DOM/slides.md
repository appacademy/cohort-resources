# Vanilla DOOM Manipulation
![vanilla doom](https://i.imgur.com/UXRkuVj.png)
---
...
* A reference to the `Document` object exists as a property on the `Window` (i.e. `window.document`) (reminder: yesterday we had `window.MovingObject`)

* DOM (Document Object Model) - A web API available for HTML, SVG, and XML documents
	* It allows programs to create, change, or remove elements from the document (aka change the webpage's structure, style, and content). 
  * The DOM represents the document using nodes and objects to represent the structure of our document (think family tree).

---
* Dom Family
![dom family meme](https://ih1.redbubble.net/image.2526068635.4924/st,small,507x507-pad,600x600,f8f8f8.jpg)

* DOM family tree
![dom family](https://www.freecodecamp.org/news/content/images/2021/09/Document.jpg)
...
* JavaScript code can react to these events via event _handlers_ or _listeners_ installed on DOM elements. (remember `document.addEveentListner` from yesterday's asteroid?)
...
[what's the difference?](https://joequery.me/code/event-target-vs-event-currenttarget-30-seconds/)
