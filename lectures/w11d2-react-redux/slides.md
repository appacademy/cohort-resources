...
+ React context allows a child component - **consumer** - to directly retrieve information from an ancestor - **provider** - no matter how far apart they are in the component tree
...
+ Takes three arguments, though only the first is required
  ...
  state tree and an action to handle
	...
	+ `enhancer`: most common use case is middleware
...
+ Hook that simply returns the `dispatch` function
