...
+ Required method in every class component
...
+ Optional method in every class component
	+ Defaults to parent `constructor` method if not defined
...
+ First argument can be an updater function or an object
	+ Callback function takes in `state` and `props` and returns an object that is then merged into the current state, overwriting shared keys
  + Object is simply merged into the current state, overwriting shared keys
+ Second argument is a completion callback that will be invoked only after state is successfully updated
...
+ Should perform any initialization that requires access to DOM
...
+ Gives access to value of `props` in previous render via `prevProps` parameter
+ Used to manipulate DOM elements or fire off fetch requests for external data
	+ *Consider navigating from a one `PostShow` page to another - we want to fetch and display new data without unmounting/remounting the component*
...
+ Should never use `setState` within
...
+ Lifecycle methods give us the ability to conditionally execute code at certain points of a component's lifecycle and in response to certain changes in state or props, just like the `useEffect` hook does
