import * as Util from "../utils/tea_api_utils";
...
export const createTea = (tea) => (dispatch) => {
  // debugger
  return Util.postTea(tea)
    .then(res => res.json())
    .then(data => dispatch(receiveTea(data)))
}

export const deleteSelectedTea = teaId => async dispatch => {
  const res = await Util.deleteTea(teaId);
  if (res.ok) {
