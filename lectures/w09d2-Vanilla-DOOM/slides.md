...
* `.getElementsByClassName(classString)` - Returns ***live*** Collection of HTMLElements with the given 
...
* `.querySelector(selector)` - Returns first element of given CSS selector

* `.querySelectorAll(selectors)` - Returns ***static*** NodeList of matches
	+ CSS selectors can be tags(div), classes(.hidden), ids, and combined(h2#TOP > p)
...
+ [Super quick and painless list of web events](https://developer.mozilla.org/en-US/docs/Web/Events#event_listing)
...
* There are 3 ways to register event handlers for a DOM element.
	* On an event target element using `EventTarget.addEventListener("event", cb)`*
...
# [Kahoot!](https://play.kahoot.it/v2/?quizId=9fc3cb0c-c6a6-4a6c-91ad-661cda26e013)