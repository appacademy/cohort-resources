...
won't worry about how the browser finds the server's IP address, or the low-level details of how a request actually makes its way to the server. We also won't concern ourselves with how the browser parses and renders the server's response. For today, we're only concerned with how these requests and responses are formatted. That's where HTTP comes in!
---
...
#### ```https://www.google.com/search?q=taylor+swift```

* The `path` in an http request is simply the path that is relative to the domain. Dont confuse path with URL
...
>>> ( We won't get into the difference between them b/c it isn't super relevant for our purposes. We'll just prefer to use PATCH in this course.)
...
![restful-routes](https://user-images.githubusercontent.com/20001191/104425438-894bb580-5535-11eb-96bd-01d7d8e05959.png)
---
