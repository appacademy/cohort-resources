...
+ Allow changes on the webpage that don't need to be saved to the database
  + From widgets: our clock's time change isn't something th DB needs any knowledge of
+ Rendering speed advantage
  + Client-side rendering (rendered html is determined by the client side)
...
## Flux
+ Frontend application architecture (state management pattern)
+ Unidirectional data flow replaces traditional MVC framework

NB: Flux is _not_ specific to React
...
+ Structure: 
  + _Store_ manages the global state of an application (can only be changed via dispatched `actions`)
  + Store shares information with the _View_ (will be React components for our usage)
  + Users can interact with Views and their behavior may dispatch _Actions_
  + When an Action is _dispatched_ the store will be updated and share changes with the View
...
![flux](https://raw.githubusercontent.com/appacademy/worldwide-lecture-notes/master/react/w10d4-redux/assets/flux_diagram.png?token=AP34ZYQR7HIYNP2IHQZMEJLBSK2AQ)
...
+ Single Source of Truth (never competing or divergent values for global state)
+ State is read-only (state can only be changed by actions)
...
![redux-diagram](https://d33wubrfki0l68.cloudfront.net/01cc198232551a7e180f4e9e327b5ab22d9d14e7/b33f4/assets/images/reduxdataflowdiagram-49fa8c3968371d9ef6f2a1486bd40a26.gif)
...
//action creator with type and tea payload
...
+ Today we dispatched actions from the console, tomorrow our _views_ will do that
![flux](https://raw.githubusercontent.com/appacademy/worldwide-lecture-notes/master/react/w10d4-redux/assets/flux_diagram.png?token=AP34ZYQR7HIYNP2IHQZMEJLBSK2AQ)
...
+ Your data is assembled from different sources and can be rendered in several places throughout the UI.
