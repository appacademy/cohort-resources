...
SELECT
  COUNT(DISTINCT type) AS types_of_possessions
FROM
  possessions;
...
SELECT
  COUNT(*) AS num_of_types
FROM
  (SELECT
    type, SUM(cost) AS sum_of_costs
  FROM
    possessions
  GROUP BY
    type
  HAVING
    SUM(cost) > 600) AS pricey_possessions;
...
SELECT
  sf_instructors.name AS owner_name, COUNT(*) AS total_possessions
FROM
  sf_instructors
JOIN
  possessions ON possessions.owner_id = sf_instructors.id
GROUP BY
  sf_instructors.name;
...
FROM
  sf_instructors
JOIN
  friendships ON friendships.sf_id = sf_instructors.id
JOIN
...
-- ACTUAL SOLUTION: All sf instructors who are pod leaders
SELECT
