...
class Farm
  ...
  def initialize(animals) # an array is still 1 argument
    @animals = animals
    @barn = Array.new(8) { NullAnimal.instance } # calls Singleton#instance
  end

  def relocate(animal, stall)
    @barn[stall] = animal
  end
...
farm.relocate(snoopy, 2)
# farm.barn[1].make_noise
farm.barn.each do |stall|
  p stall
  # stall.make_noise
end
