...
+ `Jbuilder` is a simple DSL (domain-specific language) tool to declare JSON structures 
...
Note:
* allows us to create views that render json instead of html
* curates normalized JSON response from server

---

## Jbuilder Common Methods

+ `extract!` retrieves the given attributes from an object
+ `set!` creates a key-value pair with the given name and value
+ `array!` creates an array of values with the given structure
+ `partial!` references a partial (similar to how we have used partials before)
...
## Code Demo 2 - Normalized State with Associated Data
